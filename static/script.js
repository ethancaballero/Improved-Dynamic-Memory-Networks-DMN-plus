$(function() {
    ...
    $getTaskType.on('click', function(e) {
        e.preventDefault();
        clearStory()
        getTaskType();
        getStory();
    ...
    function getTaskType() {
        $getTaskType.text('Loading...');
        $getTaskType.attr("disabled","disabled");
        $.get('/get/taskType', {taskType_i: $taskType.val()}, function(json) {
            $taskType.data('taskType_input', json["taskType"]);
        });
    }
    ...
    function getStory() {
        $.get('/get/story', function(json) {
            ...
            $answer.data('pred_answer', json["pred_answer"]);
            $getTaskType.removeAttr("disabled");
            $getTaskType.text('Load Task Type');
            //$explainTable.find('tbody').empty();
        });
    }
    ...
    function getAnswer() {
        var questionIdx       = $question.data('question_idx'),
            ...
            pred_answer       = $answer.data('pred_answer'),
